...
In this chapter, a description of the characterisation methods used to characterise the substrate surfaces is presented. %The methods are presented in the following order: optical microscopy, near-infrared transmission microscopy, \acl{sem}, \acl{eds}, \acl{afm}, and \acl{ftir}.
%%=========================================
...
%%=========================================
\section{Scanning Electron Microscopy}%\todo{Må skrive om denne seksjonen (og mer om).}
...
\Acp{se} are valence and conduction electrons emitted by atoms excited by the incident electron beam, see Fig.~\ref{fig:sem_se}. They have low energy (\SI{<50}{\electronvolt}) and emerge from very close to the sample surface, i.e. from a small interaction volume, which gives the possibility of high-resolution images. \Acp{bse} are incident electrons that scatter from the atomic nuclei and come back out of the sample, see Fig.~\ref{fig:sem_bse}. Larger atoms deflect more electrons, resulting in a larger signal at the detector. \Acp{bse} can emerge from deeper into the sample due to their high energy, i.e. from a greater interaction volume, which can result in a poorer resolution than secondary electron images. In the most common \ac{sem} mode it is the \acp{se} that are detected. The brightness of the signal increases with the number of detected \acp{se}. 
...
\begin{figure}[htbp]
    \centering
    \mySubfigure[\Acfp{se} are emitted when the atom is excited by the incident electron beam.]{0.47\linewidth}{sem_se.png}[fig:sem_se]
    \quad
    \mySubfigure[\Acfp{bse} are incident electrons that scatter from the atomic nuclei and come back out of the sample.]{0.47\linewidth}{sem_bse.png}[fig:sem_bse]
    %\quad
    \mySubfigure[X-ray fluorescence. One of two ways the atom can release excess energy after an electron from a higher energy shell falls down and fills a vacant energy level. The vacancy is formed by the emission of a secondary electron, see Fig.~ \subref{fig:sem_se}.]{0.47\linewidth}{sem_x-ray.png}[fig:sem_x-ray]
    \quad
    \mySubfigure[Auger electrons. One of two ways the atom can release excess energy after an electron from a higher energy shell falls down and fills a vacant energy level. The vacancy is formed by the emission of a secondary electron, see Fig.~ \subref{fig:sem_se}.]{0.47\linewidth}{sem_auger.png}[fig:sem_auger]
    \caption[Illustration of the various modes of electron or X-ray emission from an incident electron.]{Illustration of the various modes of electron or X-ray emission from an incident \acf{pe}: \acf{se}, \acf{bse}, \acf{ae} and X-ray fluorescence. The filled black circle represents the atomic nucleus, the circle with a minus sign represents an electron, the grey circle represents a vacant energy level, and the curly orange arrow represents the emission of an X-ray \citep[Adapted from][]{goldstein2012scanning}.}
    \label{fig:sem_ill}
\end{figure}


%%=========================================
\section{Energy Dispersive X-ray Spectroscopy}\label{sec:eds}%\todo{Må skrive om denne seksjonen.}
...
%%=========================================
\section{X-ray Photoelectron Spectroscopy}\label{sec:xps}%\todo{Må skrive om denne seksjonen.}
...
\end{equation}
where $I_\mathrm{A}/I_i$ is the measured peak area for element A/$i$ and $S_\mathrm{A}/S_i$ is the sensitivity factor of element A/$i$. The sensitivity factors take into account parameters that affect the intensity of the element, such as the distinctive cross-sections and densities of the different elements, and normalise these to that of the most intense peak. %the beam experience as it collide with the elements in the sample, that increase or decrease the probabilities for exciting a photoelectron.

A simple model for an abrupt uniform layer L on a bulk material B is used to estimate the thickness $d$ of a layer. The ratio between the signal from the layer and the signal from the bulk as a function of $d$ is given by \citet{briggs1990practical}
...
\begin{figure}[htbp]
    \centering
    \mySubfigure[The radius of the tip is similar to the radius of the objects being imaged, and the artifact is minimized.]{0.47\linewidth}{afm_artifact_small-tip.png}
    \quad
    \mySubfigure[The radius of the tip is larger than the radius of the objects being images, and the distortion is more noticeable.]{0.47\linewidth}{afm_artifact_big-tip.png}
    \caption[Illustration of the convolution effect in an \ac{afm} image due to tip size.]{Illustration of an \ac{afm} image artifact that is caused by the convolution effect between the tip and the feature which is to be visualised, i.e. a broadening effect. This image artifact is observable when using a tip with a similar or higher radius of curvature with respect to the feature. In the images, grey represents the probe tip, black represents the real surface and surface features, and red represents the displayed surface \citep[Adapted from][]{psia2002xe100}.}
...
%
%The band gap of a material can be identified where the transmission increases steeply with wavelength. This increase in transmission happens because photons with energy greater than the band gap can be absorbed by creating an electron-hole pair, while photons with energy less than the band gap cannot create an electron-hole pair and are transmitted.
%
%I tilfellet der en tynnfilm ligger på overflaten av et annet materiale, vil både toppflaten og bunnflaten av tynnfilmen reflektere lys. Den totale refleksjonen vil være summen av disse to refleksjonene. Refleksjonen kan adderes konstruktivt eller destruktivt avhengig av faseforskjellen strålene får på grunn av ulik optisk veilengde. Dette vil føre til økt refleksjon når veilengdeforskjellen tilsvarer ett helt antall bølgelengder
%\begin{equation}
%2d=m\lambda_2,
%\end{equation}
%der $d$ er tykkelsen på tynnfilmen, $m\in\mathbb{N}$ og $\lambda_2$ er bølgelengden inne i tynnfilmen gitt ved $\lambda_2=\lambda_1/n$, hvor $\lambda_1$ er bølgelengden på den innkommende strålen og $n$ er brytningsideksen til tynnfilmen. Minimum for refleksjon vil finne sted når $2d=(m+\frac{1}{2})\lambda_2$. Dermed kan interferensmønsteret, frynsene i spekteret, bli brukt for å bestemme tykkelsen av tynnfilmen
%\begin{equation}\label{eq:frynseeffekt}
%d = \frac{1}{2}\frac{1}{n\Delta k},
%\end{equation}
%hvor $n$ er brytningsideksen og $\Delta k$ er perioden for én slik frynse i frynseborden.
%
%%=========================================
